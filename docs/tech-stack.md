...
基线日期：2026-09-21，状态更新：2026-09-26。本文保留阶段 5 的技术选型；工程、真实认证、城市年份影集、原图持久保存、分页浏览、批量导入、文字和影集排序已在 T01–T07 实现，跨影集移动和地图等能力继续按任务推进。

当前状态与未完成项见 [项目总览](../README.md)。原图保存、浏览与批量异常恢复闭环已通过；完整清理、独立备份和真实规模尚待后续开发。
...
T07 复用 React Router 的 Data Router/useBlocker 管理未保存文字的站内离开确认，beforeunload 提醒硬刷新/关页；不增加依赖。同账号后台身份核验保留编辑器，退出/换号仍清空。排序采用原生 HTML 拖动及等价按钮，服务端事务操作完整影集，先写临时位置再写最终位置；文字与影集分别校验 revision，失败先查再由用户决定，不自动覆盖或重放。实现与验证边界见 [T07 记录](t07-editing-verification.md)。
...
导入使用逐文件队列，开发初始并发设为 2。首版技术参数为单张最多 50 MiB、最多 8000 万像素，接收 JPEG、PNG、静态 WebP；这些是待样本与资源实测校准的开发参数，不是用户确认的产品上限。HEIC、RAW、动画等未支持格式给出明确提示，不私自转码。T04 已用三种格式的合成样本校验原字节持久保存，T05 用小型合成 PNG 验证浏览；真实大图与规模仍须 T15 检查。
...
T05 已按 24 条资料分页实施浏览；图片进入视野上下 160px 范围才发起读取，离开范围卸载并撤销 Blob URL。同一页面最多两个原图传输；查看层只读当前一张，打开时暂停列表图片。原始字节经鉴权 fetch 取得，AbortController 与会话版本保护覆盖排队、在途和晚到响应；不把 Blob/私人列表写入浏览器持久存储。元素卸载和 URL 释放经过测试，但不等于已经测过浏览器内存峰值或大图性能。

T06 批量页复用原协议，逐文件计算 SHA256、两路 XHR 上传以取得字节进度；同样受 AbortController、CSRF 与会话版本保护。按确认的年份分组，每批最多 400 项，同影集依次提交；随机队列 UUID 与既有幂等键支持查询恢复，不在浏览器保存照片或文件名清单。401 项微型合成 PNG 的真实拆批与顺序测试、3000 项纯规划测试通过，真实不同大图的内存/耗时仍待 T15。
...
阶段 6「数据与接口设计」已形成工作基线，见 [数据与接口设计](data-api-design.md) 及其 SQL 设计样例，覆盖账号、影集、上传、原图、排序、移动、文字和回收站。阶段 7 [开发任务拆分](development-plan.md) 已形成六个本地开发批次及后续上线任务。当前完成 T01–T05，已执行的验证见 [T02 记录](t02-auth-verification.md)、[T03 记录](t03-albums-verification.md)、[T04 记录](t04-originals-verification.md) 和 [T05 记录](t05-browsing-verification.md)，从 T06 继续实施。
