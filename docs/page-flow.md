...
正式工程更新（2026-09-25）：T02 已接通真实账号，T03 已接通三城搜索、本人年份影集、重复进入与空影集保存，见 [T03 实测](t03-albums-verification.md)。T04 接通单张原图的选择→上传校验→确认保存，以及暂存恢复、重试和取消，见 [T04 实测](t04-originals-verification.md)。T05 接通每批 24 条的照片列表、按视野读取原图、年份封面、原图查看层、跨页前后切换和关闭返回位置，见 [T05 实测](t05-browsing-verification.md)。T06 在城市/影集加入独立批量核对页：文件名识别后人工核对年份和顺序，确认队列后两路上传、逐批保存，支持失败重试、停止/取消、显式部分保存及已登记批次恢复，见 [T06 实测](t06-batch-import-verification.md)。文字编辑/排序 T07、正式全国地图仍待接入，以下原型历史记录不与正式结果混用。
